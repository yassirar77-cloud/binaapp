...
class TemplateService:
    ...
    #: Prefill text for the floating WhatsApp button, by page language.
    #: The floating button used to hardcode English ("Hi, I'm interested")
    #: while the menu-card CTAs generated by the prompt were Malay — two
    #: WhatsApp entry points on one page speaking different languages.
    #: One source, language-aware.
    WHATSAPP_DEFAULT_MESSAGE = {
        "ms": "Assalamualaikum, saya berminat dengan perkhidmatan anda",
        "en": "Hi, I'm interested in your services",
    }

    @classmethod
    def whatsapp_default_message(cls, language: Optional[str] = None) -> str:
        """Language-appropriate WhatsApp prefill. Defaults to Malay — the
        product's primary market and the language of the generated pages."""
    ...
    def inject_whatsapp_button(
        ...
        default_message=None resolves from `language` so the floating button
        speaks the same language as the page's other WhatsApp CTAs.
        ...
        if not default_message:
            default_message = self.whatsapp_default_message(language)
...
<section id="delivery" style="padding:60px 20px;background:linear-gradient(135deg, #fff5eb 0%, #ffe4cc 100%);">
  <div style="max-width:1200px;margin:0 auto;">
    <div style="text-align:center;margin-bottom:40px;">
      ...
      <p style="font-size:1rem;color:#7c2d12;margin-top:24px;">
        💡 Sila gunakan butang delivery untuk membuat pesanan / Use the delivery button to place your order
      </p>
    </div>
  </div>
</section>
    ...
    def inject_integrations(
        ...
        if "whatsapp" in features and user_data.get("phone"):
            html = self.inject_whatsapp_button(
                html,
                user_data["phone"],
                user_data.get("whatsapp_message"),
