...
# ---------------------------------------------------------------------------
# Every restaurant site generated by BinaApp injects a first-party analytics
# script (see the `tracking_script` blocks below). Visitors to those sites
# never see the BinaApp dashboard, which means today they have no way to
# discover the BinaApp Privacy Policy. The merchant site is the *only*
# surface the visitor sees, so it has to carry the disclosure link.
#
# This helper appends a small subtle footer with a "Dikuasakan oleh BinaApp ·
# Polisi Privasi" link just before `</body>`. Merchants on plans with the
# `white_label` feature flag (Pro per migration 005 / 025) are exempt — the
# helper returns the HTML unchanged for them.
_ATTRIBUTION_FOOTER_HTML = (
    '\n<!-- BinaApp attribution -->\n'
    '<div style="text-align:center;padding:12px 16px;font-size:11px;'
    'color:#9ca3af;font-family:system-ui,-apple-system,sans-serif;'
    'background:transparent;">'
    'Dikuasakan oleh '
    '<a href="https://binaapp.my" style="color:#6b7280;text-decoration:none;'
    'font-weight:500;" target="_blank" rel="noopener">BinaApp</a>'
    '<span style="margin:0 6px;color:#d1d5db;">·</span>'
    '<a href="https://binaapp.my/polisi-privasi" '
    'style="color:#6b7280;text-decoration:none;" target="_blank" rel="noopener">'
    'Polisi Privasi</a>'
    '</div>\n'
)


def _inject_attribution_footer(html: str, white_label: bool = False) -> str:
    """Append the BinaApp attribution footer to a generated site.

    Returns the input unchanged when `white_label` is truthy (Pro merchants
    have removed branding as part of their plan). Otherwise inserts the
    footer just before `</body>`, falling back to a plain append if the tag
    is missing so we never silently lose the disclosure.
    """
    if white_label:
        return html
    if '</body>' in html:
...
async def _has_white_label(user_id: Optional[str]) -> bool:
    """Resolve the `white_label` plan feature flag for a user.

    Lazily imports plan_features to keep this helper close to the call
    sites that need it. Anonymous / unknown user → no white-label
    (default to showing the attribution).
    """
    if not user_id or user_id == "anonymous":
        return False
    try:
        from app.services.plan_features import get_plan_features
        features = await get_plan_features(user_id)
        return bool(features and features.get("white_label"))
    except Exception as e:
...
'''
    ...
    # default to showing the attribution — Pro merchants who reach this
    # path will still see the footer here, but the user-facing flows above
    # do resolve white_label correctly.
    final_html = _inject_attribution_footer(final_html, white_label=False)
