...
//
// By default it also renders nothing when there is no install path (no
// deferred prompt and not iOS). Set `alwaysShow` to guarantee a tappable
// affordance in that case too: it opens ManualInstallGuide (Android/desktop
// "Add to Home Screen" steps) so the button is never a dead end. Used on the
// rider login screen, where a rider on any browser must always have a way in.
...
interface InstallAppButtonProps {
  ...
  compact?: boolean;
  /**
   * Never render nothing (unless already installed/standalone). When no
   * native prompt is held and the device isn't iOS, show a button that opens
   * the manual "Add to Home Screen" guide instead of collapsing to null.
   */
  alwaysShow?: boolean;
  className?: string;
}
...
export default function InstallAppButton({
  ...
  // offered installability yet). Unless `alwaysShow` is set, render nothing
  // rather than a dead button. With `alwaysShow`, fall through and open the
  // manual "Add to Home Screen" guide on click.
  if (!canInstall && !isIOS && !alwaysShow) return null;

  const handleClick = async () => {
    if (busy) return;
    // Prefer the native dialog when a deferred prompt is held (covers
    // Chrome on iPadOS-like UAs too); fall back to the iOS guide, then to
    // the generic manual guide when neither applies.
    if (canInstall) {
      setBusy(true);
      try {
        await promptInstall();
      ...
      <button
        ...
        className={
          compact
            ...
            : `inline-flex min-h-[44px] items-center gap-1.5 rounded-full border border-white/15 bg-white/[0.06] px-4 py-2.5 text-xs font-medium text-white/80 hover:bg-white/[0.12] hover:text-white transition-colors disabled:opacity-50 ${className}`
        }
      >
