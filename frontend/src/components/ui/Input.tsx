...
const fieldBase =
  'block w-full rounded-xl border bg-white px-3.5 py-2.5 text-sm text-ink-900 ' +
...
const fieldOk = 'border-ink-200 hover:border-ink-300'
const fieldErr = 'border-err-500/60 focus:border-err-400 focus:ring-err-500/20'
...
export function Field({
  ...
  return (
    <div className={cn('flex flex-col gap-1.5', className)}>
      ...
      {children}
      {error ? (
        <p className="text-xs text-err-500">{error}</p>
      ) : hint ? (
